...
export function TechProfileModal() {
  ...
  const [name, setName] = useState(state.user?.name ?? "Tecnico");
  ...
  const availabilityTimes = state.user?.technicianProfile?.availabilityTimes ?? [];
    ...
    >
      <div className="grid gap-5">
        ...
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-(--blue-dark) text-xs font-bold text-white overflow-hidden">
            {avatarPreview ? (
        ...
        <div className="grid gap-4">
          ...
          <div className="grid gap-2">
            ...
            <div className="flex items-end justify-between gap-3">
              ...
              <button
                type="button"
                onClick={() => navigate("/tech/profile/password")}
        ...
        <div>
          <p className="m-0 text-xs font-semibold text-[#111827]">
            Disponibilidade
          </p>
          <p className="m-0 mt-1 text-xs text-[#6b7280]">
            Horarios de atendimento definidos pelo admin
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            {availabilityTimes.length ? (
              availabilityTimes.map((h) => (
                <span
                  key={h}
                  className="rounded-full border border-black/10 bg-white px-3 py-1 text-[11px] font-semibold text-[#111827]"
                >
                  {h}
                </span>
              ))
            ) : (
              <span className="text-xs text-[#6b7280]">Nao informado.</span>
            )}
          </div>
        </div>
      </div>
