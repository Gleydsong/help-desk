...
import type { Service, Ticket, TicketService, TicketStatus } from "../../services/types";
import { formatCurrencyBRL, formatDateTime, initialsFromName } from "../../utils/format";
import { StatusPill } from "../../components/ui/StatusPill";
import { TechAdditionalServiceModal } from "./TechAdditionalServiceModal";
...
export function TechTicketDetailPage() {
  ...
  useEffect(() => {
    ...
    async function loadServices() {
      ...
      try {
        const data = await api.tech.listServices(state.token);
  ...
  async function updateStatus(status: TicketStatus) {
    if (!state.token || !ticketId) return;
    try {
      const updated = await api.tech.updateTicketStatus(state.token, ticketId, status);
      setTicket(updated);
    } catch (err) {
  ...
  async function addAdditionalService(serviceId: string) {
    if (!state.token || !ticketId) return;
    try {
      const updated = await api.tech.addTicketService(state.token, ticketId, serviceId);
      setTicket(updated);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Erro ao adicionar servico.");
    }
  }
                ...
                <div className="mt-2 space-y-2">
                  {addedServices.map((s) => (
                    ...
                    >
                      ...
                      <span
                        className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-xs text-[#6b7280]"
                        aria-label="Servico adicional"
                      >
